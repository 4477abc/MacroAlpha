...
The queries compute various financial ratios dynamically, including the Interest Coverage Ratio (ICR) defined as $ICR = \frac{EBITDA}{Interest Expense}$, Gross Margin calculated as $\frac{Gross Profit}{Revenue}$ or equivalently $1 - \frac{COGS}{Revenue}$, the Debt-to-Revenue Ratio given by $\frac{Total Debt}{Revenue}$, and ROE along with other profitability ratios derived from combinations of these fundamental fields. This design approach---storing raw data and computing ratios in queries---maintains data integrity, enables flexible analytical queries, and ensures consistent ratio definitions across different use cases.
...
\textbf{Result:} The contemporaneous covariance analysis between sector monthly returns and CPI changes reveals distinct patterns. Energy and Materials sectors show positive covariance with CPI changes, suggesting they move concurrently with inflation. Consumer Staples shows weak positive covariance, while Technology shows near-zero covariance. The covariance measure indicates directional co-movement: positive covariance means sector returns tend to rise when CPI increases. Note: covariance values are scale-dependent and not normalized; the sign indicates directional relationship but magnitude is not directly comparable across sectors. Monthly returns are calculated using arithmetic approximation (weekly returns $\times$ 4.33).
