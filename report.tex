...
\textbf{Result:} The covariance analysis between sector returns and CPI changes (contemporaneous and lagged) reveals distinct patterns. Energy and Materials sectors show positive covariance with contemporaneous CPI changes, suggesting they move with inflation rather than predicting it. Consumer Staples shows weak positive covariance, while Technology shows near-zero covariance. The lagged analysis reveals that Energy and Materials sectors show stronger positive covariance with future CPI than with current CPI, suggesting they may provide early inflation signals. Note: covariance values are scale-dependent; the sign indicates directional relationship but magnitude is not directly interpretable as correlation coefficients.

\textbf{Analysis:}

Energy and Materials sectors' positive covariance with future CPI reflects their role in the inflation transmission mechanism. Energy price increases flow through to transportation costs, heating costs, and manufacturing inputs, eventually appearing in CPI with a lag. Materials price increases flow through to finished goods prices. These sectors showing stronger covariance with future CPI than current CPI suggests commodity price movements provide early signals of inflation trends, even if the signals are noisy. This finding supports research on yield curve predictors \cite{ang2009} and inflation forecasting, which document how asset prices can signal future macroeconomic conditions.
...
To confirm data integrity, we verified that continuously listed blue-chip companies maintain complete 20-year records. Apple (AAPL), Microsoft (MSFT), and JP Morgan (JPM) all show uninterrupted annual data from 2005 through 2024, confirming that the aggregate coverage decline stems from company exits rather than systematic data gaps.
